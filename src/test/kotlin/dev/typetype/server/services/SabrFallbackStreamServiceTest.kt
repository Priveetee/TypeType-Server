...
class SabrFallbackStreamServiceTest {
    ...
    @Test
    fun `recovers youtube extraction failure with live hls`() = runTest {
        val delegate = mockk<StreamService>()
        val sessionStore = mockk<SabrSessionStore>()
        val tokenSessionClient = mockk<TypetypeTokenYoutubeSessionClient>()
        val liveFallback = mockk<StreamService>()
        ...
        coEvery { liveFallback.getStreamInfo(YOUTUBE_URL) } returns ExtractionResult.Success(live)
        val service = SabrFallbackStreamService(delegate, sessionStore, tokenSessionClient, liveFallback)

        val result = service.getStreamInfo(YOUTUBE_URL)

        assertEquals(ExtractionResult.Success(live), result)
        coVerify(exactly = 0) { tokenSessionClient.fetchPlaybackSession(any()) }
    }

    @Test
    fun `recovers live response without hls from classic fallback`() = runTest {
        val delegate = mockk<StreamService>()
        val sessionStore = mockk<SabrSessionStore>()
        val tokenSessionClient = mockk<TypetypeTokenYoutubeSessionClient>()
        val liveFallback = mockk<StreamService>()
        val incomplete = testStreamResponse().copy(isLive = true, isLiveContent = true)
        val live = incomplete.copy(
            hlsUrl = "https://example.com/live.m3u8",
            hasLiveManifest = true,
        )
        coEvery { delegate.getStreamInfo(YOUTUBE_URL) } returns ExtractionResult.Success(incomplete)
        coEvery { sessionStore.fetchInfo(VIDEO_ID, cachedFirst = true) } returns null
        coEvery { liveFallback.getStreamInfo(YOUTUBE_URL) } returns ExtractionResult.Success(live)
        val service = SabrFallbackStreamService(delegate, sessionStore, tokenSessionClient, liveFallback)
    ...
    @Test
    fun `rejects classic vod as live fallback`() = runTest {
        val delegate = mockk<StreamService>()
        val sessionStore = mockk<SabrSessionStore>()
        val tokenSessionClient = mockk<TypetypeTokenYoutubeSessionClient>()
        val liveFallback = mockk<StreamService>()
        val failure = ExtractionResult.Failure("blocked")
        coEvery { delegate.getStreamInfo(YOUTUBE_URL) } returns failure
        coEvery { sessionStore.fetchInfo(VIDEO_ID, cachedFirst = true) } returns null
        coEvery { liveFallback.getStreamInfo(YOUTUBE_URL) } returns ExtractionResult.Success(testStreamResponse())
        coEvery { tokenSessionClient.fetchPlaybackSession(VIDEO_ID) } returns null
        val service = SabrFallbackStreamService(delegate, sessionStore, tokenSessionClient, liveFallback)
