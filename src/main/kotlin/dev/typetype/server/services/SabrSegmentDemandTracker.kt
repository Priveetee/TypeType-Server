...
internal object SabrSegmentDemandTracker {
    ...
    fun request(holder: SabrSessionHolder, request: SabrSegmentRequest, registeredAtMs: Long): Unit {
        ...
        if (holder.session.isBeyondEnd(request) && !holder.isFutureLiveRequest(request)) return clear(holder, request)
    ...
    fun next(holder: SabrSessionHolder): SabrSegmentRequest? {
        ...
        for ((key, value) in demands) {
            ...
            if (holder.session.getCachedSegment(request) != null ||
                holder.session.isBeyondEnd(request) && !holder.isFutureLiveRequest(request)
            ) {
