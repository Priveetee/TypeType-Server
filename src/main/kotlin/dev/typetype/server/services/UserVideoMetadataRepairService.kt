...
class UserVideoMetadataRepairService(private val resolver: VideoMetadataResolver) {
    suspend fun repairUserLists(userId: String): Int {
        val urls = candidateUrls(userId)
    ...
    private suspend fun candidateUrls(userId: String): List<String> = DatabaseFactory.query {
        val playlistUrls = PlaylistVideosTable.selectAll()
        ...
        val watchLaterUrls = WatchLaterTable.selectAll()
        ...
        val favoriteUrls = FavoritesTable.selectAll()
        ...
        (playlistUrls + watchLaterUrls + favoriteUrls).distinct()
    }
