...
fun Route.blockedRoutes(blockedService: BlockedService, token: String) {
    get("/blocked/channels") {
        call.withAuth(token) { call.respond(blockedService.getChannels()) }
    }
    ...
    get("/blocked/videos") {
        call.withAuth(token) { call.respond(blockedService.getVideos()) }
    }
    post("/blocked/videos") {
        call.withAuth(token) {
            val item = runCatching { call.receive<BlockedItem>() }.getOrElse {
