...
internal suspend fun ApplicationCall.respondProxyResult(result: ExtractionResult<ProxyResponse>): Unit = when (result) {
    is ExtractionResult.Success -> respondProxy(result.data)
...
private suspend fun ApplicationCall.respondProxy(proxy: ProxyResponse): Unit {
    try {
